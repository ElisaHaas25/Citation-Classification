...
with open("output_zeroshot_6cat.csv", mode="w", newline="", encoding="utf-8") as f:

    writer = csv.writer(f)

    writer.writerow(["bibcode","citation_context_no" ,"citation_context","target_reference" ,"distance_used_score", "methodology_score", "background_score", "criticism_score", "comparison_score", "extension_score"])
    ...
    for i in range(len(pdfs)):  
        ...
        for j in range(len(contexts)):
            ...
            background_score = scores_dict[classification_categories[2]]
            criticism_score = scores_dict[classification_categories[3]]
            comparison_score = scores_dict[classification_categories[4]]
            extension_score = scores_dict[classification_categories[5]]

            
            # Determine predicted label      
            
            writer.writerow([filename.replace(".pdf", ""), j+1, context, ref_keys_found[j],  data_usage_score, methodology_score, background_score, criticism_score, comparison_score, extension_score])
            
            print(" ")
            print("distance_used score:", data_usage_score)
            print("methodology_score:", methodology_score)
            print("background_score:", background_score)
            print("criticism_score:", criticism_score)
            print("comparison_score:", comparison_score)
            print("extension_score:", extension_score)
