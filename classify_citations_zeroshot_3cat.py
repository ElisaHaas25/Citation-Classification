...
classification_categories = [
    ...
    "The authors cite Bailer-Jones et al. for methodology, they either use, discuss, extend or alter the method by Bailer-Jones et al.",
...
with open("output_zeroshot_3cat.csv", mode="w", newline="", encoding="utf-8") as f:

    writer = csv.writer(f)

    writer.writerow(["bibcode", "citation_context_no","citation_context","target_reference", "predicted_label","distance_used_score", "methodology_score", "other_score"])
    ...
    for i in range(len(pdfs)):  
        ...
        # 4. extract context for each reference key if found
        ...
        for j in range(len(contexts)):
            ...
            methodology_score = scores_dict[classification_categories[1]]
            other_score = scores_dict[classification_categories[2]]
            
            # Determine predicted label
            
            predicted_label = max(scores_dict, key=scores_dict.get)

            if predicted_label == classification_categories[0]:
                predicted_label_short = "distance_used"
            elif predicted_label == classification_categories[1]:
                predicted_label_short = "methodology"
            else:
                predicted_label_short = "other"
            
            writer.writerow([filename.replace(".pdf", ""), j+1,context ,ref_keys_found[j],predicted_label_short,  data_usage_score, methodology_score, other_score])
            
            print(" ")
            print("distance usage score:", data_usage_score)
            print("methodology score:", methodology_score)
            print("others score:", other_score)
            print("Prediction:", predicted_label_short)
